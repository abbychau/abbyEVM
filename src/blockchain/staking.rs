...
impl StakingManager {
    ...
    pub fn stake(
        ...
        self.stakes.entry(staker).or_default().push(stake_info.clone());
        
        // Add to validator's delegations
        self.validator_delegations.entry(validator).or_default().push(stake_info);
        ...
        log::info!("Staked {} Abby tokens from {} to validator {}", 
                  self.format_abby_amount(amount), staker, validator);
    ...
    pub fn unstake(
        ...
        let staker_stakes = self.stakes.get_mut(&staker)
        ...
        log::info!("Unstaked {} Abby tokens from validator {} by {}", 
                  self.format_abby_amount(amount), validator, staker);
    ...
    pub fn withdraw(
        ...
        log::info!("Withdrew {} Abby tokens for {} from validator {}", 
                  self.format_abby_amount(withdrawn_amount), staker, validator);
    ...
    pub fn claim_rewards(
        ...
        if total_rewards > U256::zero() {
            ...
            log::info!("Claimed {} Abby token rewards for {} from validator {}", 
                      self.format_abby_amount(total_rewards), staker, validator);
        }
    ...
    pub fn get_staker_info(&self, staker: &Address) -> Vec<&StakeInfo> {
        self.stakes.get(staker).map(|v| v.iter().collect()).unwrap_or_default()
    }
    
    pub fn get_validator_delegations(&self, validator: &Address) -> Vec<&StakeInfo> {
        self.validator_delegations.get(validator).map(|v| v.iter().collect()).unwrap_or_default()
    }
    
    pub fn get_total_staked_to_validator(&self, validator: &Address) -> U256 {
        self.validator_delegations.get(validator)
            .map(|stakes| stakes.iter().map(|s| s.amount).fold(U256::zero(), |acc, x| acc + x))
            .unwrap_or(U256::zero())
    }
    ...
    pub fn create_validator(
        ...
        // Self-stake
        self.stake(validator_address, validator_address, initial_stake, consensus)?;
        
        log::info!("Created validator {} with {} Abby tokens stake", 
                  validator_address, self.format_abby_amount(initial_stake));
