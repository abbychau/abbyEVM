...
impl CodeGenerator {
    ...
    fn visit_program(&mut self, program: &Program) -> CompileResult<()> {
        ...
        for stmt in &program.statements {
            if let Statement::FuncDecl(func_decl) = stmt {
                self.functions
                    .insert(func_decl.name.clone(), self.bytecode.len() as u16);
            }
        }
    ...
    pub fn visit_expression(&mut self, expr: &Expression) -> CompileResult<()> {
        match expr {
            ...
            Expression::MemoryAssignment(mem_assign) => {
                self.visit_memory_assignment_expr(mem_assign)
            }
            Expression::StorageArrayAssignment(storage_assign) => {
                self.visit_storage_array_assignment_expr(storage_assign)
            }
        }
    }
            ...
            Expression::MemberAccess(member) => {
                ...
                if let Expression::Variable(obj) = &*member.object {
                    if obj.name == "console" {
                        ...
                        return Err(CompileError {
                            message: format!(
                                "Member access not supported for object: {}",
                                obj.name
                            ),
                        });
                    }
    ...
    fn visit_memory_assignment_expr(
        &mut self,
        mem_assign: &MemoryAssignmentExpr,
    ) -> CompileResult<()> {
    ...
    fn visit_storage_array_assignment_expr(
        &mut self,
        storage_assign: &StorageArrayAssignmentExpr,
    ) -> CompileResult<()> {
    ...
    fn place_label(&mut self, label: &str) {
        self.jump_labels
            .insert(label.to_string(), self.bytecode.len() as u16);
