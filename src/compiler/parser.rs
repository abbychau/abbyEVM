...
impl Parser {
        ...
        if self.match_token(&TokenType::LeftBracket) {
            ...
            match name.as_str() {
                ...
                "memory" => Ok(Statement::ExprStmt(ExprStmt {
                    expression: Expression::MemoryAccess(MemoryAccessExpr::Store(
                        Box::new(index),
                        Box::new(value),
                    )),
                })),
            ...
            match expr {
                ...
                Expression::ArrayAccess(array_access) => {
                    if let Expression::Variable(var) = *array_access.object {
                        match var.name.as_str() {
                            "storage" => Ok(Expression::storage_array_assignment(
                                *array_access.index,
                                value,
                            )),
                            "memory" => {
                                // memory[offset] = value -> memory.store(offset, value)
                                Ok(Expression::MemoryAccess(MemoryAccessExpr::Store(
                                    Box::new(*array_access.index),
                                    Box::new(value),
                                )))
                            }
    ...
    fn factor(&mut self) -> ParseResult<Expression> {
        ...
        while let Some(op) =
            self.match_binary_op(&[TokenType::Slash, TokenType::Star, TokenType::Percent])
        {
            ...
            if let TokenType::Identifier(method) = &token.token_type {
                ...
                match method_name.as_str() {
                    "get" => {
                        ...
                        Ok(Expression::StorageAccess(StorageAccessExpr::Get(Box::new(
                            key,
                        ))))
                    }
                    "set" => {
                        ...
                        Ok(Expression::StorageAccess(StorageAccessExpr::Set(
                            Box::new(key),
                            Box::new(value),
                        )))
                    }
            ...
            if let TokenType::Identifier(method) = &token.token_type {
                ...
                match method_name.as_str() {
                    "load" => {
                        ...
                        Ok(Expression::MemoryAccess(MemoryAccessExpr::Load(Box::new(
                            offset,
                        ))))
                    }
                    "store" => {
